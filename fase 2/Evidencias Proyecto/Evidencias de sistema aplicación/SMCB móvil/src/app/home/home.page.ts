...
export class HomePage implements OnInit, OnDestroy {
  ...
  async showDayMantenciones(day: any) {
    ...
    const buttons = day.events.map((event: any) => ({
      text: `${event.nombremantencion} - ${formatTime(event.fechahora)}`,
      cssClass: `urgencia-${event.nivelurgencia.toLowerCase().replace(' ', '-')}`,
      handler: () => {
        this.goToMantencionDetalle(event.id);
      }
    }));
    ...
    const alert = await this.alertController.create({
      header: `Mantenciones del día ${day.day}`,
      cssClass: 'mantenciones-alert',
      buttons: buttons
    });
